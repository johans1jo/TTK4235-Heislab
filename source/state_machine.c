...
int run(){
    ...
    while (1){
        ...
        switch (elev_state){
            ...
            else if (orders_above() == 1 || orders_above() == 2){
            ...
            else if (orders_bellow() == -1 || orders_bellow() == 2){
            ...
            case RUNNING    :
            if (order_at_floor(dir, motor_dir) == 1 || (elev_get_floor_sensor_signal() != -1 && e_stopped == 1)){
                motor_dir = DIRN_STOP;
                e_stopped = 0;
                elev_set_motor_direction(DIRN_STOP);
                elev_set_door_open_lamp(1);
            ...
            if (time_out() == 1){
                ...
                else if (dir == DIRN_DOWN && orders_bellow() == 2){
                    motor_dir = DIRN_DOWN;
                    elev_set_motor_direction(DIRN_DOWN);
                    printf("DIRN_DOWN -- BELLOW 2\n");
                    elev_state = RUNNING;
                }
                else if (dir == DIRN_UP && orders_above() == 2){
                    motor_dir = DIRN_UP;
                    elev_set_motor_direction(DIRN_UP);
                    printf("DIRN_UP -- ABOVE 2\n");
                    elev_state = RUNNING;
                }
                else if (dir == DIRN_UP && orders_above() == 1){
                    motor_dir = DIRN_UP;
                    elev_set_motor_direction(DIRN_UP);
